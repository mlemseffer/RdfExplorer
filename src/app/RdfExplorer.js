...
import { exampleQueries } from '../data/exampleQueries.js';
import { parseTurtle } from '../services/parser.js';
import { runSparqlRequest, convertSparqlResultsToTriples } from '../services/sparql.js';
import {
  extractLabel,
  categorizeType,
  inferLiteralType,
...
export class RdfExplorer {
  ...
  setupEventListeners() {
    ...
    this.startNodeInput.addEventListener('input', () =>
      this.showAutocomplete(this.startNodeInput, 'start')
    );
    ...
    this.endNodeInput.addEventListener('input', () =>
      this.showAutocomplete(this.endNodeInput, 'end')
    );
    ...
    document.getElementById('prevPathBtn').addEventListener('click', () => {
      ...
      this.currentPathIndex =
        (this.currentPathIndex - 1 + this.allPaths.length) % this.allPaths.length;
      this.highlightPath(this.allPaths[this.currentPathIndex]);
      document.getElementById('pathCounter').textContent =
        `${this.currentPathIndex + 1} / ${this.allPaths.length}`;
    });
    ...
    // Clustering — utiliser le <select id="clusterModeSelect"> déjà présent dans le HTML
    const clusterSelect = document.getElementById('clusterModeSelect');
    if (clusterSelect) {
      ...
      console.warn('[Clustering] #clusterModeSelect introuvable dans le DOM.');
    }
  }
  ...
  async parseWithN3(content) {
    //Mode d'emploi :
    // Parse un fichier RDF au format Turtle (délégué au service parser.js)
  ...
  buildGraphFromTriples(triples) {
    ...
    nodeMap.forEach((node) => {
      if (node.type === 'unknown') {
        node.type = inferDefaultType(node.id);
      }
    });
  ...
  categorizeType(typeUri) {
    return categorizeType(typeUri);
  }

  inferLiteralType(predicate) {
    return inferLiteralType(predicate);
  }
  ...
  renderGraph() {
    ...
    const predicateFilteredLinks = sourceLinks.filter((l) =>
      this.activePredicates.has(l.predicate)
    );

    const anchorNodeId =
      this.isSubgraphMode && this.subgraphRootNode
        ? this.subgraphRootNode.id
        : this.startNode
          ? this.startNode.id
          : null;
    ...
    const visibleNodes = nodeCandidates.filter((n) => {
      return (
        !this.hideIsolatedNodes || usedNodeIds.has(n.id) || (anchorNodeId && n.id === anchorNodeId)
      );
    });
    ...
    const sizeScale =
      this.nodeSizeMode === 'fixed'
        ? () => 12
        : d3.scaleLinear().domain(d3.extent(sourceNodes, sizeAccessor)).range([8, 30]);
    ...
    this.simulation.on('tick', () => {
      ...
      this.svg
        .selectAll('.zoom-group .links text')
      ...
      if (this.showNodeLabels) {
        this.svg
          .selectAll('.zoom-group .nodes text')
          .attr('x', (d) => d.x)
          .attr('y', (d) => d.y);
      }
  ...
  dragged(event, d) {
    ...
    if (this.simulationPaused) {
      ...
      this.svg
        .selectAll('.zoom-group .links line')
  ...
  resetGraphView() {
    ...
    this.svg.selectAll('.zoom-group .links line').attr('stroke-width', 2);
  ...
  findShortestPath() {
    ...
    for (const nodeId of visibleNodeIds) {
      ...
      if (
        (direction === 'Sortantes' || direction === 'Entrantes + Sortantes') &&
        this.adjList.has(nodeId)
      ) {
      ...
      if (
        (direction === 'Entrantes' || direction === 'Entrantes + Sortantes') &&
        this.revAdjList.has(nodeId)
      ) {
  ...
  highlightPath(path) {
    ...
    this.svg
      ...
      .attr('stroke', (d) => {
        const src = typeof d.source === 'object' ? d.source.id : d.source;
        const tgt = typeof d.target === 'object' ? d.target.id : d.target;
        return pathLinks.has(`${src}->${tgt}`) || pathLinks.has(`${tgt}->${src}`)
          ? '#003366'
          : '#9ca3af';
      })
      .attr('stroke-width', (d) => {
  ...
  getColorScale() {
    ...
    if (this.nodeColorMode === 'type') {
      ...
      const compIds = Array.from(
        new Set(this.visibleNodes.map((n) => this.componentIdByNode.get(n.id)))
      ).sort((a, b) => a - b);
      ...
      return d3.scaleLinear().domain([0, maxDegree]).range(['#F1A7A7', '#580E0E']);
    }
  }
  ...
  showSubgraph() {
    ...
    while (queue.length > 0) {
      ...
      this.graph.links.forEach((link) => {
        const source =
          typeof link.source === 'object' ? link.source : this.nodeMap.get(link.source);
        const target =
          typeof link.target === 'object' ? link.target : this.nodeMap.get(link.target);
    ...
    const newVisibleLinks = this.graph.links.filter(
      (l) =>
  ...
  updateEdgeColors() {
    ...
    if (this.edgeColorMode === 'predicate') {
      ...
      linkSelection.transition().duration(300).attr('stroke', '#9ca3af');
  ...
  hideCurrentlyIsolatedNodes() {
    ...
    this.visibleNodes.forEach((n) => {
      const hasLink = this.visibleLinks.some(
        (l) =>
  ...
  async runSparqlRequest(query) {
    ...
    try {
      const json = await runSparqlRequest(endpointUrl, query, this.currentAbortController.signal);
  ...
  convertSparqlResultsToTriples(results, isExpand = false, nodeId = null) {
    return convertSparqlResultsToTriples(results, isExpand, nodeId);
  }
  ...
  async enrichWithTypes(newTriples) {
    ...
    const valuesClause = Array.from(uniqueNodes)
      .map((uri) => `<${uri}>`)
      .join(' ');
    const typeQuery = `
  ...
  async addAllNeighborsOfSelectedNode() {
    ...
    if (!this.isSparqlGraph) {
      alert(
        "Le graphe actuel ne provient pas d'une requête SPARQL. Impossible d'ajouter des voisins."
      );
      return;
    }
      ...
      this.graph.nodes
        .filter(
          (n) =>
            ...
            this.activeTypes.has(n.type) ||
            n.id === this.startNode.id
        )
        .filter((n) => !this.hiddenNodes.has(n.id))
        .filter(
          (n) => n.inDegree + n.outDegree >= this.minDegreeFilter || n.id === this.startNode.id
        )
        .map((n) => n.id)
    );
  ...
  renderTree(hierarchyData) {
    ...
    this.svg.append('g').attr('class', 'zoom-group').attr('transform', 'translate(50,50)');
    ...
    const sizeScale =
      this.nodeSizeMode === 'fixed'
        ? () => 12
        : d3.scaleLinear().domain(d3.extent(this.graph.nodes, sizeAccessor)).range([8, 30]);
  ...
  applyClusteringForces(simulation, width, height) {
    ...
    let needRecompute =
      clusters.length !== this.clusterCenters.size || clusters.some((k) => !prevKeys.has(k));
